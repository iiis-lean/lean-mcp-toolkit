...
from ....config import DiagnosticsConfig, ToolchainConfig
...
class LeanCommandRuntime:
    ...
    def __post_init__(self) -> None:
        self._lean_sem = threading.BoundedSemaphore(
            max(1, self.diagnostics_config.max_concurrent_lean_checks)
        )
        self._lake_sem = threading.BoundedSemaphore(
            max(1, self.diagnostics_config.max_concurrent_lake_build)
        )
    ...
    def _maybe_semaphore(self, sem: threading.BoundedSemaphore):
        if not self.diagnostics_config.enable_concurrency_limits:
            return nullcontext()
