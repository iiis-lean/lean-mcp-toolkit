"""Contracts for proof_search_alt.state_search."""
...
class ProofSearchAltStateSearchRequest(DictModel):
    ...
    @classmethod
    def from_dict(cls, data: JsonDict) -> "ProofSearchAltStateSearchRequest":
        return cls(
            project_root=(
...
class StateSearchItem(DictModel):
    ...
    @classmethod
    def from_dict(cls, data: JsonDict) -> "StateSearchItem":
        raw_payload = data.get("raw_payload")
        return cls(
            name=str(data.get("name") or ""),
            formal_type=str(data["formal_type"]) if data.get("formal_type") is not None else None,
...
class ProofSearchAltStateSearchResponse(DictModel):
    ...
    provider: str = "state_search"
    goal: str | None = None
    backend_mode: str = "remote"
    items: tuple[StateSearchItem, ...] = field(default_factory=tuple)
    count: int = 0

    @classmethod
    def from_dict(cls, data: JsonDict) -> "ProofSearchAltStateSearchResponse":
        items = tuple(
        ...
        return cls(
            ...
            provider=str(data.get("provider") or "state_search"),
