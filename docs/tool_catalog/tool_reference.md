...
| `lsp.declaration_soundness_batch` | Inspect multiple explicit declarations in one request without discovering the whole repository. | Local implementation |  | Imports the distinct explicit modules once, runs all exact `#print axioms` commands in one temporary LSP probe, and returns results in request order; optional source files are separate scan evidence. |
| `lsp.compiled_declaration_batch` | Inspect exact constants from the compiled Lean Environment without claiming source extraction. | Local implementation |  | Imports distinct explicit modules once and returns exact owner, ConstantInfo kind, signature, universe arity, and a canonical `#check _root_.Target` reference command in request order. Core Environment evidence identifies equation theorems, inductive constructors/recursors, and structure projections; optional `to_additive` provenance uses Mathlib translation state. Unavailable or unknown provenance is non-fatal to compiled identity. |
...
`lean_explore.find` remains a fuzzy discovery tool by default. Callers that already know a complete declaration name may pass `exact_name`; the response then contains only records whose full `name` equals that value. A related fuzzy candidate is never substituted for a missing exact declaration.
